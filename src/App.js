...
export default function App() {
  ...
  const filteredAdmins = useMemo(() => {
    ...
    return allAdmins.filter(adm => 
        ...
        (adm.role?.toLowerCase().includes(lowSearch))
    );
  ...
  const statistics = useMemo(() => {
    ...
    const relevantReports = adminData?.role === "super" 
        ? reports 
        : reports.filter(r => r.store === adminData?.assignedStore);

    if (relevantReports.length === 0) return null;

    const totalAudits = relevantReports.length;
    const avgScore = Math.round(relevantReports.reduce((acc, r) => acc + (r.yesCount / r.totalQuestions), 0) / totalAudits * 100);
    ...
    relevantReports.forEach(r => {
        ...
        if (!inspectorStats[r.name]) inspectorStats[r.name] = { count: 0, score: 0 };
        inspectorStats[r.name].count++;
        inspectorStats[r.name].score += (r.yesCount / r.totalQuestions);
    ...
    const topInspectorEntry = Object.entries(inspectorStats).sort((a,b) => (b[1].score/b[1].count) - (a[1].score/a[1].count))[0];
    ...
    return {
        ...
        topInspector: topInspectorEntry ? { name: topInspectorEntry[0], score: Math.round((topInspectorEntry[1].score/topInspectorEntry[1].count)*100) } : null,
        frequentFailures
    };
  }, [reports, adminData]);
  ...
  const createAdmin = async () => {
    ...
    try {
      ...
      setUserActionState({ loading: false, error: err.message, success: false });
    }
  };
        ...
        <div style={{display:'flex', flexDirection:'column', gap:'20px'}}>
          ...
          <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'12px'}}>
             ...
             <div style={{display:'flex', gap:'8px', flex:1, minWidth:'300px'}}>
                <div className="card" style={{margin:0, padding:'8px 16px', display:'flex', alignItems:'center', gap:'12px', flex:1}}>
                    <Search size={20} color="var(--text-secondary)"/>
                    <input 
                        className="input-field" 
                        style={{background:'none', border:'none', padding:0}} 
                        placeholder="Search reports (store # or name)..." 
                        value={searchTerm}
                        onChange={e => {
                ...
                {adminData?.role === 'super' && (
                    <button 
                        className={`nav-btn ${searchTerm === user.email ? 'active' : ''}`}
                        title="Filter your own audits"
                        onClick={() => {
                            setSearchTerm(prev => prev === user.email ? "" : user.email);
                            setCurrentPage(1);
                        }}
                    >
                        <Filter size={18}/>
          ...
          {statistics && (
            <div className="card" style={{margin: 0, padding: '20px'}}>
                <h3 style={{marginTop: 0, display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '15px'}}>
                    <BarChart3 size={20} color="var(--accent-blue)"/> Store Insights
                </h3>
                <div className="stats-summary" style={{display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px'}}>
                    <div className="stat-card">
                        <div style={{fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700}}>TOTAL AUDITS</div>
                        <div style={{fontSize: '1.5rem', fontWeight: 800, marginTop: '4px'}}>{statistics.totalAudits}</div>
                        <div style={{fontSize: '0.65rem', color: 'var(--success)', display: 'flex', alignItems: 'center', gap: '2px', marginTop: '4px'}}>
                            <TrendingUp size={10}/> Data Feed
                        </div>
                    </div>
                    <div className="stat-card">
                        <div style={{fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700}}>AVG COMPLIANCE</div>
                        <div style={{fontSize: '1.5rem', fontWeight: 800, marginTop: '4px', color: getPercentageColor(statistics.avgScore)}}>{statistics.avgScore}%</div>
                        <div className="mini-progress" style={{marginTop: '8px'}}>
                            <div className="mini-fill" style={{width: `${statistics.avgScore}%`, background: getPercentageColor(statistics.avgScore)}}></div>
                        </div>
                    </div>
                    {adminData?.role === 'super' ? (
                        <>
                        <div className="stat-card">
                            <div style={{fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700}}>TOP STORE</div>
                            <div style={{fontSize: '1.1rem', fontWeight: 800, marginTop: '4px', color: 'var(--accent-orange)'}}>#{statistics.topStore?.id}</div>
                            <div style={{fontSize: '0.65rem', opacity: 0.7}}>{statistics.topStore?.score}% Avg</div>
                        </div>
                        <div className="stat-card">
                            <div style={{fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700}}>TOP INSPECTOR</div>
                            <div style={{fontSize: '1.1rem', fontWeight: 800, marginTop: '4px', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{statistics.topInspector?.name}</div>
                            <div style={{fontSize: '0.65rem', opacity: 0.7}}>{statistics.topInspector?.score}% Avg</div>
                        </div>
                        </>
                    ) : (
                        <div className="stat-card" style={{gridColumn: 'span 2'}}>
                            <div style={{fontSize: '0.7rem', color: 'var(--text-secondary)', fontWeight: 700}}>CRITICAL ATTENTION NEEDED</div>
                            <div style={{display:'flex', flexDirection:'column', gap:'4px', marginTop:'6px'}}>
                                {statistics.frequentFailures.length > 0 ? statistics.frequentFailures.map(([task, count], i) => (
                                    <div key={i} style={{fontSize: '0.7rem', display:'flex', justifyContent:'space-between', background:'rgba(255,77,79,0.1)', padding:'4px 8px', borderRadius:'4px'}}>
                                ...
                                )) : <span style={{fontSize:'0.7rem', opacity:0.5}}>No recurring failures detected</span>}
                            </div>
                        </div>
                    )}
                </div>
          ...
          {adminData?.role === "super" && (
            ...
            <div className="admin-grid" style={{display: 'grid', gridTemplateColumns: '1fr 1.5fr', gap: '16px'}}>
              <div className="card" style={{margin: 0}}>
                ...
                <div className="scroll-custom" style={{maxHeight:'250px', overflowY:'auto', background:'rgba(0,0,0,0.2)', borderRadius:'12px'}}>
                  {dynamicQuestions.map((q, i) => (
              ...
              <div className="card" style={{margin:0, display:'flex', flexDirection:'column'}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px'}}>
                    ...
                    <div style={{display:'flex', alignItems:'center', gap:'8px', background:'rgba(0,0,0,0.2)', padding:'4px 10px', borderRadius:'10px'}}>
                        <Search size={14} opacity={0.5}/>
                        <input 
                            placeholder="Find manager..." 
                            className="input-field" 
                            style={{fontSize:'0.75rem', padding:'4px', border:'none', background:'none', width:'120px'}}
                ...
                <div style={{display:'flex', flexDirection:'column', gap:'8px', marginBottom:'15px', padding:'10px', background:'rgba(255,255,255,0.02)', borderRadius:'12px'}}>
                  <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px'}}>
                    <input className="input-field" style={{fontSize:'0.85rem'}} placeholder="Email" value={newAdminEmail} onChange={e => setNewAdminEmail(e.target.value)} />
                    <input className="input-field" style={{fontSize:'0.85rem'}} type="password" placeholder="Pass" value={newAdminPass} onChange={e => setNewAdminPass(e.target.value)} />
                  </div>
                  <div style={{display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:'8px'}}>
                    <select className="input-field" style={{fontSize:'0.85rem'}} value={newAdminRole} onChange={e => setNewAdminRole(e.target.value)}>
                    ...
                    />
                    <button className="nav-btn primary" disabled={userActionState.loading} onClick={createAdmin}>
                        {userActionState.loading ? <Loader2 className="animate-spin" size={16}/> : <UserPlus size={16}/>}
                    </button>
                  </div>
                  {userActionState.error && <div style={{color:'var(--danger)', fontSize:'0.7rem', textAlign:'center'}}>{userActionState.error}</div>}
                  {userActionState.success && <div style={{color:'var(--success)', fontSize:'0.7rem', textAlign:'center'}}>User created!</div>}
                </div>

                <div className="scroll-custom" style={{maxHeight:'150px', overflowY:'auto', background:'rgba(0,0,0,0.2)', borderRadius:'12px'}}>
                   {filteredAdmins.length > 0 ? filteredAdmins.map(adm => (
                     <div key={adm.id} style={{padding:'10px', display:'flex', justifyContent:'space-between', alignItems:'center', borderBottom:'1px solid #ffffff08', fontSize:'0.85rem'}}>
                        <div style={{display:'flex', flexDirection:'column', overflow:'hidden'}}>
                          <span style={{fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}} title={adm.email}>{adm.email}</span>
                   ...
                   )) : <div style={{padding:'20px', textAlign:'center', fontSize:'0.8rem', opacity:0.5}}>No users match search</div>}
                </div>
              </div>
            </div>
          ...
          {totalPages > 1 && (
            <div className="pagination-bar">
              ...
              <div style={{display:'flex', gap:'8px', alignItems:'center'}}>
                 <span style={{fontWeight: 700, fontSize: '1rem', color:'var(--accent-blue)'}}>{currentPage}</span>
                 <span style={{opacity:0.5}}>/</span>
                 <span style={{opacity:0.8}}>{totalPages}</span>
              </div>
              <button 
                className="nav-btn" 
          ...
          {filteredReports.length === 0 && (
            <div style={{textAlign:'center', padding:'40px', opacity:0.5}}>No reports found for the current search or filters.</div>
          )}
        </div>
      )}
      ...
      {selectedReport && (
        <div className="modal-overlay" onClick={() => setSelectedReport(null)}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="scroll-custom" style={{display:'flex', flexDirection:'column', gap:'10px', maxHeight:'400px', overflowY:'auto'}}>
